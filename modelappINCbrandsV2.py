...
# -------------------------
# Custom CSS Injection for Modern Look
# -------------------------
st.markdown(
    """
    <style>
...
st.markdown("<h1 class='header-font'>Supply Chain & Inventory Model</h1>", unsafe_allow_html=True)
...
interest_rate = st.sidebar.number_input(
    "Actual Interest Rate (%)", min_value=0.0, max_value=100.0, value=5.0, step=0.1
)
service_level = st.sidebar.slider(
    "Required Service Level (0-1)", min_value=0.0, max_value=1.0, value=0.95
)
layout_type = st.sidebar.radio(
    "Layout Type", options=["Central and Fronts", "Main Regionals"]
)
if layout_type == "Main Regionals":
    st.sidebar.info("With 'Main Regionals', all warehouses must be of type MAIN.")
unit_cost = st.sidebar.number_input(
    "Unit (4 Panels) Cost ($)", min_value=0.0, value=80.0, step=1.0
)

# Calculate Z_value using norm.ppf (logic remains unchanged)
Z_value = norm.ppf(service_level)

# -------------------------
# Global Data Containers and Constants
# -------------------------
market_area_data = {}   # To store market area parameters for each brand
warehouse_data = []     # To store warehouse setup details
...
# -------------------------
# Organize the App Using Tabs
# -------------------------
tabs = st.tabs(["Setup", "Calculations", "Results"])
...
with tabs[0]:
    ...
    st.markdown("<h2 class='subheader-font'>Rental Parameters</h2>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        sq_ft_per_unit = st.number_input(
            "Square feet required per unit (4 Panels)",
            min_value=0.0,
            value=0.8,
            step=0.1,
            format="%.1f"
    ...
    with col2:
        overhead_factor_main = st.number_input(
            "Overhead factor for MAIN warehouse",
            min_value=1.0,
            value=1.2,
            step=0.1,
            format="%.1f"
    ...
    overhead_factor_front = st.number_input(
        "Overhead factor for FRONT warehouse",
        min_value=1.0,
        value=1.5,
        step=0.1,
        format="%.1f"
    ...
    with st.expander("Market Areas Setup"):
        ...
        # Initialize market_area_data structure for each selected market area
        for area in selected_market_areas:
            st.markdown(f"### Parameters for Market Area: {area}")
            brand_data = {}
            for brand in BRANDS:
    ...
    with st.expander("Warehouse Setup"):
        ...
        for i in range(int(num_warehouses)):
            st.markdown(f"### Warehouse {i+1}", unsafe_allow_html=True)
            ...
            # Additional shipping inputs for MAIN warehouses in Main Regionals
            land_shipping_data = {}
            ...
            if wh_type == "MAIN":
                ...
                main_wh_options = []
                main_wh_mapping = {}
                # Look for existing MAIN warehouses for FRONT warehouse assignment
    ...
    # Additional Validation: Ensure every market area is served by at least one warehouse
    st.markdown("<h3>Validation</h3>", unsafe_allow_html=True)
    market_not_served = []
...
with tabs[1]:
    st.markdown("<div class='section'>", unsafe_allow_html=True)
    st.markdown("<h2 class='subheader-font'>Cost Calculations</h2>", unsafe_allow_html=True)
        ...
        if layout_type == "Central and Fronts":
            ...
            if main_wh is None:
                ...
                if layout_type == "Central and Fronts":
                    ...
                    for wh in warehouse_data:
                        if wh["type"] == "FRONT":
                            for area in wh["served_markets"]:
                                front_total += compute_daily_demand_sum_for_area(area)
                    agg_safety += 12 * front_total
                agg_avg_inventory = (agg_annual / 12.0) + agg_safety
                agg_fin_cost = agg_avg_inventory * 1.08 * (interest_rate / 100.0) * unit_cost

                st.subheader("Aggregated Inventory Financing Results")
                st.write(f"Total Annual Forecast (all areas): {agg_annual:.2f} units")
                st.write(f"Aggregated Safety Stock: {agg_safety:.2f} units")
                ...
                for brand in BRANDS:
                    ...
                    total_brand_avg += bdata["avg_inventory"]
                    total_brand_fin += bdata["financing_cost"]
                    total_brand_safe += bdata["safety_stock"]
                st.write("---")
                st.write(f"**Total (Brands Combined):**")
                st.write(f"  - Average Inventory: {total_brand_avg:.2f} units")
                st.write(f"  - Safety Stock: {total_brand_safe:.2f} units")
                st.write(f"  - Financing Cost (per year): ${total_brand_fin:.2f}")
            ...
            for wh in warehouse_data:
                if wh["type"] == "MAIN":
                    ...
                    wh_std = 0
                    for area in wh["served_markets"]:
                        wh_std += compute_std_sum_for_area(area)
                    safety_stock_wh = wh_std * sqrt(LT) * Z_value
                    avg_inventory_wh = (wh_annual / 12.0) + safety_stock_wh
                    overall_avg_inventory += avg_inventory_wh
                    overall_safety += safety_stock_wh
            total_avg_inventory = overall_avg_inventory
            total_safety = overall_safety
            financing_cost = overall_avg_inventory * 1.08 * (interest_rate / 100.0) * unit_cost
            st.subheader("Inventory Financing Results (Main Regionals)")
            st.write(f"Overall Safety Stock: {total_safety:.2f} units")
            st.write(f"Overall Average Inventory: {total_avg_inventory:.2f} units")
            st.write(f"Overall Financing Cost (per year): ${financing_cost:.2f}")
...
with tabs[2]:
    st.markdown("<div class='section'>", unsafe_allow_html=True)
    st.markdown("<h2 class='subheader-font'>Results & Visualizations</h2>", unsafe_allow_html=True)
    # Placeholder for interactive visualizations (e.g., Plotly charts)
    st.write("Interactive charts and detailed result summaries will appear here.")
